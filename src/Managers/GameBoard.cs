	...
	namespace GameLogic.Managers
	{
		public class GameBoard
		{
			...
			//public
	            ...
	            {
	                PlayerManager.PManager.addPlayer(name);
	            }
	
	            public Player changePlayerName(string name)
	            {
	                return PlayerManager.PManager.changePlayerName(name);
	            }
	
	            public Thing getRandomThingFromCup()
	            {
	                return ThingManager.TManager.getRandomThingFromCup();
	            }
	
	            public Thing getThingFromBank(string type)
	            {
	                return ThingManager.TManager.getThingFromBank(type);
	            }
	
	            public bool setPlayerOrder()
	            {
	                return PlayerManager.PManager.setPlayerOrder();
	            }
	
	            public List<Player> getPlayers()
	            {
	                return PlayerManager.PManager.getPlayers();
	            }
	
	            public Phase getCurrentPhase()
	            {
	                return PhaseManager.PhManager.getCurrentPhase();
	            }
	
	            public Player getPlayer(string name)
	            {
	                return PlayerManager.PManager.getPlayer(name);
	            }

                public Player getPlayerByNumber(int i)
                {
                    return PlayerManager.PManager.getPlayerByNumber(i);
                }
			...
			//private
				...
				private GameBoard () 
	            {
	                ...
	                List<Player> players = PlayerManager.PManager.getPlayers();
				...
				{
					TestCode.initSpecialCharacters(ThingManager.TManager.getBank());
				}
	        
	            private void initCup()
	            {
	                TestCode.initCreatures(ThingManager.TManager.getCup());   
	            }
